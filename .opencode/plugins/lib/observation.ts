...
const SECRET_VALUE_RE = new RegExp(
  `(${SECRET_KEY_RE.source})` +
  `(?:["'\\s:=]+)` +
  `((?:[A-Za-z]+\\s+)?[A-Za-z0-9_\\-/.+=]{8,})`,
  "gi"
)
...
function getGitRemoteUrl(projectRoot: string): string {
  try {
...
function detectProject(worktree: string): ProjectInfo {
...
function scrubArgs(args: Record<string, unknown> | undefined): string | null {
  if (!args) return null
  try {
