...
export const ECCHooksPlugin = async ({ client, $, directory, worktree, }) => {
    ...
    return {
        ...
        "experimental.chat.messages.transform": async (_input, output) => {
            ...
            if (firstUser.parts.some((p) => p.type === "text" && (p.text.includes("ECC_EXTREMELY_IMPORTANT") || p.text.includes("###ECC_INSTINCTS_END###"))))
                return;
...
${content}
            ...
            const INSTINCT_CLI = process.env.ECC_INSTINCT_CLI || path.join(projectRoot, "skills", "continuous-learning-v2", "scripts", "instinct-cli.py");
            let identityBlock = "";
            try {
            ...
            try {
                ...
                const allInstincts = JSON.parse(result);
                const highConf = allInstincts
                    .filter((i) => typeof i.confidence === "number" && i.confidence >= 0.7)
                ...
                if (highConf.length > 0) {
                    ...
                    for (const inst of highConf) {
                        const line = `- ${inst.domain} (${Math.round(inst.confidence * 100)}%): ${inst.trigger}\n`;
                        if ((block + line + "###ECC_INSTINCTS_END###").length > 2000)
                            break;
                        block += line;
                    }
                    block += "\n###ECC_INSTINCTS_END###";
        ...
        "tool.execute.after": async (input, output) => {
            ...
            if (hookEnabled("post:observe", ["standard", "strict"]) && input.tool !== "bash") {
                try {
                    recordObservation(worktreePath, input.tool, input.args);
                }
                catch {
                    // Observation failed silently
                }
        ...
        "session.deleted": async () => {
            ...
            if (hookEnabled("session:analyze", ["standard", "strict"]) && process.env.ECC_SESSION_ANALYSIS_ENABLED !== "false") {
                ...
                try {
                    ...
                    log("warn", `[ECC] Session analysis skipped: ${e.message}`);
                }
            }
